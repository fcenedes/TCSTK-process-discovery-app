...
import {
  LiveAppsConfig, Groups, Roles, LiveAppsService, RouteAccessControlConfig
} from '@tibco-tcstk/tc-liveapps-lib';
import { Claim, Sandbox, MessageQueueService } from '@tibco-tcstk/tc-core-lib';
...
export class PdCaseComponent implements OnInit {
    ...
    constructor(private liveapps: LiveAppsService, private router: Router, private route: ActivatedRoute, private messageService: MessageQueueService) { }
    ...
    ngOnInit() {
        ...
        this.liveapps.caseSearch(this.route.snapshot.data.caseDataHolder.untaggedCasedataObj.DataSourceId, +this.sandbox.id, '33', '1', 0, 5, 0).pipe(
            map(cases => {

            })
        );

        this.spotfirePath = spotfireConfig.analysisPath +  '_' + this.route.snapshot.data.caseDataHolder.untaggedCasedataObj.DataSourceId;
        this.spotfireFilter =
        [
            {
                dataColumnName: spotfireConfig.columnNames[0],
                dataTableName: 'cases', // spotfireConfig.tableName,
                filterType: 'ListBoxFilter',
                filterSettings: {
                    values: filterString
                }
            }
